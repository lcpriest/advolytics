...
Advocately.prototype.survey = function(userId, traits) {
  ...
  return this._post('survey', data).then(function(response) {
    if (response.config) {
      ...
      self._sideLoadJs('http://d1m5z9tultw2jo.cloudfront.net/advonps.js');
      self._sideLoadCss('http://d1m5z9tultw2jo.cloudfront.net/advonps.css');
...
// Advocately.prototype.get = function(endpoint, params) {
//   var data = this._buildMessage('get', this._normalize(params, endpoint));

//   if (typeof this.apiToken !== 'undefined' && this.apiToken.trim().length > 0) {
//     fetchival(this._buildUrl(endpoint)).get(data).then(function(response) {
//       if (response !== 'success') {
//         this._printError(response);
//       }
//     });
//   } else {
//     this._printError('Missing apiToken');
